...
if (__name__ == "__main__"):
    ...
    # Check to see if the files necessary for the INDEL module are in the sandbox
    if (platform.system() == "Windows"):
        # Check to see if INDEl files are already in the sandbox
        if (not os.path.isfile(homedir + "\\InteractiveROSETTA\\iRosetta_Lookup.exe")):
            try:
                # executable
                source = os.getcwd() + "\\bin\\iRosetta_Lookup_Win64.exe"
                destination = homedir + "\\InteractiveROSETTA\\iRosetta_Lookup.exe"
                os.rename(source, destination)

                # pdblist.dat
                pdb_source = os.getcwd() + "\\data\\pdblist.dat"
                pdb_destination = homedir + "\\.InteractiveROSETTA/pdblist.dat"
                os.rename(pdb_source, pdb_destination)

                # looplist.dat
                loop_source = os.getcwd() + "\\data\\looplist.dat"
                loop_destination = homedir + "\\.InteractiveROSETTA\\looplist.dat"
                os.rename(loop_source, loop_destination)

                # grid.dat
                grid_source = os.getcwd() + "\\data\\grid.dat"
                grid_destination = homedir + "\\.InteractiveROSETTA\\grid.dat"
                os.rename(grid_source, grid_destination)

            except:
                print "Failed to copy INDEL lookup executable"

    if (platform.system() == "Darwin"):
        if (not os.path.isfile(homedir + "/.InteractiveROSETTA/iRosetta_Lookup.exe")):
            try:
                # executable
                source = os.getcwd() + "/bin/iRosetta_Lookup_osx.exe"
                destination = homedir + "/.InteractiveROSETTA/iRosetta_Lookup.exe"
                os.rename(source, destination)
        ...
        if (not os.path.isfile(homedir + "/.InteractiveROSETTA/iRosetta_Lookup.exe")):
            try:
                # executable
                exe_source = os.getcwd() + "/bin/iRosetta_Lookup_Linux.exe"
