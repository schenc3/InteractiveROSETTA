...
f.close()
# If the minimum number of nodes is greater than the total available, exit with an error
if (total_cpus < mincpus):
    fout = open("errreport", "w")
    fout.write("This server does not have enough nodes to complete minimum jobs\n")
    fout.write("Please notify the system administrator of this problem\n")
    fout.write("Minimum Number of Nodes for Any Job: " + str(mincpus) + "\n")
    fout.write("Total Nodes Available: " + str(total_cpus) + "\n")
    fout.close()
    exit()
...
# Append the expiration date for these data to the scoring file so the client knows when the results
# will not be available anymore
if (scorefile):
    fhScore = open(scorefile, "a")
    expiration = datetime.datetime.now() + datetime.timedelta(weeks=2)
    fhScore.write("EXPIRATION\t" + expiration.strftime("%Y-%m-%d") + "\n")
    fhScore.close()
