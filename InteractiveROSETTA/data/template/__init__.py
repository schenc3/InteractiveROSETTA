...
### Replace Example below with the name of your module
...
class ModulePanel(wx.lib.scrolledpanel.ScrolledPanel):
	...
	# The locations of the data, image, and help directories for this module
	if (platform.system() == "Windows"):
	    self.datadir = os.path.expanduser("~") + "\\InteractiveROSETTA\\modules\\" + thisfile.split("__init__.py")[0] + "\\data"
	    self.imagedir = os.path.expanduser("~") + "\\InteractiveROSETTA\\modules\\" + thisfile.split("__init__.py")[0] + "\\images"
	    self.helpdir = os.path.expanduser("~") + "\\InteractiveROSETTA\\modules\\" + thisfile.split("__init__.py")[0] + "\\help"
	else:
	    self.datadir = os.path.expanduser("~") + "/.InteractiveROSETTA/modules/" + thisfile.split("__init__.py")[0] + "/data"
	    self.imagedir = os.path.expanduser("~") + "/.InteractiveROSETTA/modules/" + thisfile.split("__init__.py")[0] + "/images"
	    self.helpdir = os.path.expanduser("~") + "/.InteractiveROSETTA/modules/" + thisfile.split("__init__.py")[0] + "/help"
	...
	### If the above is true, it will not run unless it can send the job to the server
	### If False, you can still conditionally allow the user to send it to a server, but
	###     if it fails, then it will attempt to run the same job locally
	...
	if (platform.system() == "Windows"):
	    ...
	    self.lblLabel = wx.StaticBitmap(self, -1, wx.Image(self.imagedir + "/lblLabel.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap(), pos=(10, 80), size=(320, 20))
	else:
	...
	if (platform.system() == "Darwin"):
	    self.btnButton = wx.BitmapButton(self, id=-1, bitmap=wx.Image(self.imagedir + "/btnButton_1.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap(), pos=(110, 140), size=(100, 25))
	else:
	...
	self.btnBitmapButton = wx.BitmapButton(self, -1, wx.Image(self.imagedir + "/colorwheel.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap(), pos=(110, 170), size=(100, 25))
	...
	### STANDARD TEXT BUTTON, THIS EXAMPLE SETS UP A FUNCTION TO TOGGLE BETWEEN STATES
	if (platform.system() == "Darwin"):
	    self.btnServer = wx.BitmapButton(self, id=-1, bitmap=wx.Image(self.imagedir + "/btnServer_Off.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap(), pos=(110, 430), size=(100, 25))
	else:
	...
	if (platform.system() == "Darwin"):
	    self.btnSubmit = wx.BitmapButton(self, id=-1, bitmap=wx.Image(self.imagedir + "/btnSubmit.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap(), pos=(110, 460), size=(100, 25))
	else:
	...
	# automatically scroll to include whatever element had the focus, which gets incredibly
	...
	if (self.toggleState == 1):
	    self.toggleState = 2
	    if (platform.system() == "Darwin"):
		self.btnButton.SetBitmapLabel(bitmap=wx.Image(self.imagedir + "/btnButton_2.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap())
	    else:
		self.btnButton.SetLabel("State 2")
	elif (self.toggleState == 2):
	    self.toggleState = 3
	    if (platform.system() == "Darwin"):
		self.btnButton.SetBitmapLabel(bitmap=wx.Image(self.imagedir + "/btnButton_3.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap())
	    else:
		self.btnButton.SetLabel("State 3")
	else:
	    self.toggleState = 1
	    if (platform.system() == "Darwin"):
		self.btnButton.SetBitmapLabel(bitmap=wx.Image(self.imagedir + "/btnButton_1.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap())
	    else:
	...
	if (self.serverOn):
	    self.serverOn = False
	    if (platform.system() == "Darwin"):
		self.btnServer.SetBitmapLabel(bitmap=wx.Image(self.imagedir + "/btnServer_Off.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap())
	    else:
		self.btnServer.SetLabel("Server Off")
	else:
	    self.serverOn = True
	    if (platform.system() == "Darwin"):
		self.btnServer.SetBitmapLabel(bitmap=wx.Image(self.imagedir + "/btnServer_On.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap())
	    else:
	...
	if (self.buttonState == "Submit!"):
	    ...
	    self.stage = 1
	    if (platform.system() == "Darwin"):
		self.btnSubmit.SetBitmapLabel(bitmap=wx.Image(self.parent.parent.scriptdir + "/images/osx/btnSubmit_Cancel.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap())
	    else:
	    ...
	    dlg.Destroy()
	    if (platform.system() == "Darwin"):
		self.btnSubmit.SetBitmapLabel(bitmap=wx.Image(self.imagedir + "/osx/btnSubmit.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap())
	    else:
		self.btnSubmit.SetLabel("Submit!")
	    self.buttonState = "Submit!"
